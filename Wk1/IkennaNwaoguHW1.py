...
counterObject = counterObject(flower_orders)
...
colors = []  # This probably not an efficient way to run this. Looked to utilize itertools but none of functions I think works.
for i in flower_orders:
    colors = i.split('/') + colors


counterColors = {}
for i in set(colors):
    count = colors.count(i)
    counterColors.update({i:count})

#counterObjectColors = counterObject(colors) # I can''t seem to figure out why am getting an error calling the function counterObject function.

plt.bar(counterColors.keys(),counterColors.values())
plt.show()

# 4 combinations of 2
result = itertools.combinations(flower_orders,2)

# for item in result:  
#     print(item)
# # 5 combinations of 3
# result = itertools.combinations(flower_orders,3)
# for item in result:  
#     print(item)

# 6 Dictionary used to plot histogram of colors
print(counterColors)

# 7 Probability of colors based on how often they occur.
total = 0
for value in counterColors:
    total = total + (counterColors[value])

print(total)

probability = {}
for key in counterColors:
    probability.update({key:(counterColors[key]/total)})
...
separator = ', '
print(separator.join(dead_men_tell_taies))

# separate = [separator.join(dead_men_tell_taies)]
#print(separate)

print((separator.join(dead_men_tell_taies)).replace(" ", ""))